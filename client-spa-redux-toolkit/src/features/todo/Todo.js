...
function Todo() {
    ...
    const doGetCreateTodo = async () => {
        ...
        doShowModal();
        await dispatch(clearTodoItemData());
    };
    ...
    const doGetAllTodos = useCallback(async () => {
        ...
        await dispatch(fetchGetAllTodos());
    }, [dispatch]);
    ...
    const doGetEditTodo = async (id) => {
        ...
        doShowModal();
        await dispatch(fetchGetTodo(id));
    };
    ...
    const doCreateTodo = async (e) => {
        ...
        await dispatch(
            fetchCreateTodo({
                title: allData.item.title,
                description: allData.item.description,
            })
        );
    ...
    const doUpdateTodoItems = async () => {
        ...
        await dispatch(clearTodoItems());

        await doGetLimitedTodos(0);
    };
    ...
    const doSearch = async (e) => {
        ...
        if (e.charCode === 13) {
            ...
            await doGetLimitedTodos(0, e.target.value);
        }
    };
    ...
    useEffect(() => {
        setNoMoreItems(allData.noMoreItems);
    }, [allData.noMoreItems]);
